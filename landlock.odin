...
/*
    Package landlock restricts a program's ability to use files and networking.

    # Restricting file access

    The following invocation will restrict all threads so that they
    ...
    once. The effect is the same as calling RestrictPaths and RestrictNet
    one after another, but it happens in one step.
    ...
    landlock.V5.RestrictPaths(...) (without the call to BestEffort)
    enforces the given rules using the capabilities of Landlock V5,
    but returns an error if that functionality is not available on the
    system that the program is running on.
    ...
    When using the Odin Landlock package, callers need to identify at
    which ABI level they want to use Landlock and call one of the
    restriction methods (e.g. RestrictPaths) on the corresponding ABI constant.
    ...
    the BestEffort method to gracefully degrade to using the best
    available Landlock version on the current kernel.

    In this case, the Odin Landlock library will enforce as much as
